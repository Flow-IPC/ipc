...
#if JEM_ELSE_CLASSIC
  /* Instructed to do so by set_logger() public docs (short version: SHM-jemalloc's singletons are global things,
   * and it would not be cool for ipc::session non-global objects to impose their individual loggers on them). */
